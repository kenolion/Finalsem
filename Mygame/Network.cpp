...
int Network::resolveAddressAndPort()
{
	...
	if (iResult != 0) {
		...
		return 1;
		//ERROR CHECKING, IF FAIL, WSACleanup and stop program.
	}
...
{
	if (listen(ListenSocket, SOMAXCONN) == SOCKET_ERROR) //Makes socket into a listening state.
	{
...
int Network::receiveUntilPeerShutdown()
{
...
int Network::sendAndReceiveDataAsClient()
{
...
int Network::disconnectClientSocket()
{// shutdown the send half of the connection since no more data will be sent
	iResult = shutdown(ConnectSocket, SD_SEND);
	if (iResult == SOCKET_ERROR) {
		printf("shutdown failed: %d\n", WSAGetLastError());
